...
char *argv[];
{
  ...
  in = fopen("geom_database/mushroom.geom","r");
  if (in == NULL) {
  ...
  RiPixelFilter(RiBoxFilter,1.,1.);
    ...
    RiDisplay("pilz.rle",RI_FILE,RI_RGB,RI_NULL);
    ...
    RiWorldBegin();
      ...
      RiColor(green);
      Ka=.2; Kd=1.; Ks=.0;
      RiSurface("clouds","Ka",&Ka,"Kd",&Kd,"Ks",&Ks,RI_NULL);
      RiPolygon(4,RI_P,(RtPointer)boden,RI_NULL);
