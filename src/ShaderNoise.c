/* \subsection{ShaderNoise.c} \label{ShaderNoise.c}
   Implementation der Noise-Routinen, die von den Shadern 
   aufgerufen werden k"onnen. Diese Routienen wurden urspr"unglich 
   von Robert Skinner, Craig E. Kolb und F. Kenton Musgrave
   geschrieben.
\begin{verbatim}*/
#include <stdio.h>
#include <math.h>
...
double drand48();
/*
 * nrand() returns a uniformly distributed 
...
#define Hash3d(a,b,c) \
        hashTable[hashTable[hashTable[(a) & 0xfff] ^ \
                  ((b) & 0xfff)] ^ ((c) & 0xfff)]
#define Hash(a,b,c) \
       (xtab[(xtab[(xtab[(a) & 0xff] ^ (b)) & 0xff] \
             ^ (c)) & 0xff] & 0xff)

#define INCRSUM(m,s,x,y,z)	((s)*(RTable[m]*0.5 \
        + RTable[m+1]*(x) + RTable[m+2]*(y) \
        + RTable[m+3]*(z)))
...
RtPoint v;
{
	...
	return Crc16(v, sizeof(RtPoint));
}

/*
 * Note that passing a double to Crc16 and interpreting it as
 * an array of chars means that machines with different 
 * floating-point representation schemes will evaluate 
 * Noise(point) differently.
 */
static int
Crc16(buf, count)
register char *buf;
 ...
 * Vector-valued "Noise"
 */
Rm_noiseVV(point, result)
RtPoint point, result;
{
	register int	ix, iy, iz, jx, jy, jz;
	double		x, y, z;
	double px, py, pz, s;
...
_init_ShaderNoise()
{
        ...
        hashTable = (short int *) RmMalloc(4096*
                               sizeof(short int));
        for (i = 0; i < 4096; i++)
                hashTable[i] = i;
        for (i = 4095; i > 0; i--) {
